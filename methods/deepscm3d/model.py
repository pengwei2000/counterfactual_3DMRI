...
class SCM(nn.Module):
    ...
    def __load_parameters(self):
        ...
        for model in self.models.values():
            ...
            if model.name == 'image_hvae':
                file_name = 'image_hvae-epoch=21-val_loss=2.028-batch_size=2.ckpt'
            elif model.name == 'image_vae':
                file_name = 'image_vae-epoch=281-val_loss=2.090-batch_size=16.ckpt'
            elif model.name == 'image_gan':
                file_name = 'image_gan-epoch=39-val_loss=7.782-batch_size=2.ckpt'
            print(file_name)
            device = "cuda"
        ...
        for var in self.graph_structure.keys():
            if repl is None or var not in repl.keys():
                if len(self.graph_structure[var]) == 0:
                    ...
                    if var == 'image':
                        xs[var] = self.models[var].decode(us[var][0], torch.cat([xs[pa] for pa in self.graph_structure[var]], dim=1),us[var][1],us[var][2],us[var][3],us[var][4],us[var][5])
                    else:
