...
OUR_SIZE = 176
config = 'counterfactual-benchmark/counterfactual_benchmark/methods/deepscm3d/configs/adni3d/gan.json'
assert os.path.isfile(config), f"{config} is not a file"
...
from Model_HA_GAN_144_192_144 import Discriminator, Generator, Encoder, Sub_Encoder
...
parser.add_argument('--exp-name', default='HA_GAN_condition_on_anatomy', type=str,
                    help='name of the experiment')
parser.add_argument('--fold', default=0, type=int,
                    help='fold number for cross validation')
# configs for conditional generation
parser.add_argument('--lambda-class', default=0.2, type=float,
...
def main():
    ...
    if args.continue_iter != 0:
        ckpt_path = '/mnt/disks/VLM/Pengwei/ha-gan/checkpoint/'+args.exp_name+'/G_iter'+str(args.continue_iter)+'.pth'
        ckpt = torch.load(ckpt_path, map_location='cuda')
        ckpt['model'] = trim_state_dict_name(ckpt['model'])
        G.load_state_dict(ckpt['model'])
        g_optimizer.load_state_dict(ckpt['optimizer'])
        ckpt_path = '/mnt/disks/VLM/Pengwei/ha-gan/checkpoint/'+args.exp_name+'/D_iter'+str(args.continue_iter)+'.pth'
        ckpt = torch.load(ckpt_path, map_location='cuda')
        ckpt['model'] = trim_state_dict_name(ckpt['model'])
        D.load_state_dict(ckpt['model'])
        d_optimizer.load_state_dict(ckpt['optimizer'])
        ckpt_path = '/mnt/disks/VLM/Pengwei/ha-gan/checkpoint/'+args.exp_name+'/E_iter'+str(args.continue_iter)+'.pth'
        ckpt = torch.load(ckpt_path, map_location='cuda')
        ckpt['model'] = trim_state_dict_name(ckpt['model'])
        E.load_state_dict(ckpt['model'])
        e_optimizer.load_state_dict(ckpt['optimizer'])
        ckpt_path = '/mnt/disks/VLM/Pengwei/ha-gan/checkpoint/'+args.exp_name+'/Sub_E_iter'+str(args.continue_iter)+'.pth'
        ckpt = torch.load(ckpt_path, map_location='cuda')
        ckpt['model'] = trim_state_dict_name(ckpt['model'])
    ...
    summary_writer = SummaryWriter("/mnt/disks/VLM/Pengwei/ha-gan/checkpoint/"+args.exp_name)
    ...
    for iteration in range(args.continue_iter, args.num_iter):
        ...
        assert real_images.shape[2:] == (144, 176, 144)
        D.zero_grad()
        real_images = real_images.float().cuda()
        class_label = class_label.cuda()
        # low-res full volume of real image
        real_images_small = F.interpolate(real_images, scale_factor = 0.25) # downsample 4 times 144,176,144 -> 36,44,36
        
        # randomly select a high-res sub-volume from real image
        crop_idx = np.random.randint(0,144*5/6) # 256 * 7/8 + 1 # TODO: HARDCODED 128//8
        real_images_crop = real_images[:,:,crop_idx:crop_idx+144//6,:,:] # batch, 1, 24,176,144
        ...
        if args.num_class == 0: # unconditional
            ...
            # GAN loss + auxiliary regressor loss
            d_real_loss = loss_f(y_real_pred, real_labels) + \
                          loss_mse(y_real_class, class_label)

            # random generation
            noise = torch.randn((args.batch_size, args.latent_dim)).cuda()
            fake_images, fake_images_small = G(noise, crop_idx=crop_idx, class_label=class_label)
        ...
        for iters in range(args.g_iter):
            ...
            if args.num_class == 0: # unconditional
                ...
                g_loss = loss_f(y_fake_g, real_labels) + \
                         args.lambda_class * loss_mse(y_fake_g_class, class_label)
        ...
        with torch.no_grad():
            ...
            z_hat = torch.cat(z_hat_i_list, dim=2).detach()   # 36,44,36
        sub_z_hat = Sub_E(z_hat)
        # Reconstruction
        if args.num_class == 0: # unconditional
            sub_x_hat_rec, sub_x_hat_rec_small = G(sub_z_hat, crop_idx=crop_idx)
        else: # conditional
            sub_x_hat_rec, sub_x_hat_rec_small = G(sub_z_hat, crop_idx=crop_idx, class_label=class_label)
        ...
        if (iteration+1) >= 10000 and (iteration+1) % 5000 == 0 and count < 10:
            featmask = np.squeeze((0.5*fake_images[0]+0.5).data.cpu().numpy())
            featmask = nib.Nifti1Image(featmask.transpose((2,1,0)),affine = np.eye(4))
            nib.save(featmask, f'models\ha-gan\HAGAN_sample_training_{count}_epoch_{iteration}.nii.gz')
            count += 1
            print("Sample saved!")

        if iteration > 10000 and (iteration+1)%500 == 0:
            torch.save({'model':G.state_dict(), 'optimizer':g_optimizer.state_dict()},'/mnt/disks/VLM/Pengwei/ha-gan/checkpoint/'+args.exp_name+'/G_iter'+str(iteration+1)+'.pth')
            torch.save({'model':D.state_dict(), 'optimizer':d_optimizer.state_dict()},'/mnt/disks/VLM/Pengwei/ha-gan/checkpoint/'+args.exp_name+'/D_iter'+str(iteration+1)+'.pth')
            torch.save({'model':E.state_dict(), 'optimizer':e_optimizer.state_dict()},'/mnt/disks/VLM/Pengwei/ha-gan/checkpoint/'+args.exp_name+'/E_iter'+str(iteration+1)+'.pth')
            torch.save({'model':Sub_E.state_dict(), 'optimizer':sub_e_optimizer.state_dict()},'/mnt/disks/VLM/Pengwei/ha-gan/checkpoint/'+args.exp_name+'/Sub_E_iter'+str(iteration+1)+'.pth')
