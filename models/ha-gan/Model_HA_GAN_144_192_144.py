...
class Sub_Encoder(nn.Module): # [64, 32, 44, 36]
    def __init__(self, channel=256, latent_dim=1024):
        ...
        self.relu = nn.Tanh()
        self.conv2 = nn.Conv3d(channel//4, channel//4, kernel_size=4, stride=2, padding=1) # out:[16,22,18]
        self.bn2 = nn.GroupNorm(8, channel//4)
        self.conv3 = nn.Conv3d(channel//4, channel//2, kernel_size=4, stride=2, padding=1) # out:[8,11,9]
...
class Encoder(nn.Module):
    def __init__(self, channel=64): #input: 24,176,144 TODO
        super(Encoder, self).__init__()

        self.relu = nn.Tanh()
        self.conv1 = nn.Conv3d(1, channel//2, kernel_size=4, stride=2, padding=1) # in:[16,128,128], out:[8,64,64] ours:[12,88,72]
        self.bn1 = nn.GroupNorm(8, channel//2)
        self.conv2 = nn.Conv3d(channel//2, channel//2, kernel_size=3, stride=1, padding=1) # out:[8,64,64] #ours: [12,88,72]
        self.bn2 = nn.GroupNorm(8, channel//2)
        self.conv3 = nn.Conv3d(channel//2, channel, kernel_size=4, stride=2, padding=1) # out:[4,32,32] #[6,44,36]
...
class Sub_Discriminator(nn.Module): # input: 36, 44, 36
    def __init__(self, num_class=0, channel=256):
        super(Sub_Discriminator, self).__init__() #TODO
        self.channel = channel
        self.num_class = num_class

        self.conv2 = SNConv3d(1, channel//4, kernel_size=4, stride=2, padding=1) # out:[16,16,16] #[18, 22, 18]
        self.conv3 = SNConv3d(channel//4, channel//2, kernel_size=4, stride=2, padding=1) # out:[8,8,8] #[9,11,9]
    ...
    def forward(self, h):
        ...
        if self.num_class == 0:
            ...
            h = self.pool(self.conv5(h)).view((-1,1+self.num_class))  # bz, num_class+1
            return h[:,:1], h[:,1:]
...
class Discriminator(nn.Module): 
    def __init__(self, num_class=0, channel=512):
        ...
        self.num_class = num_class
        # D^H
        self.conv2 = SNConv3d(1, channel//16, kernel_size=4, stride=2, padding=1) # out:[8,64,64,64]  # [32,12,88,72]
        self.conv3 = SNConv3d(channel//16, channel//8, kernel_size=4, stride=2, padding=1) # out:[4,32,32,32] # [64,6,44,36]
        self.conv4 = SNConv3d(channel//8, channel//4, kernel_size=(2,4,4), stride=(2,2,2), padding=(0,1,1)) # out:[2,16,16,16] #[3,22,18]
        self.conv5 = SNConv3d(channel//4, channel//2, kernel_size=(2,4,4), stride=(2,2,2), padding=(0,1,1)) # out:[1,8,8,8] #[1,1,11,9]
        self.conv6 = SNConv3d(channel//2, channel, kernel_size=(1,4,4), stride=(1,2,2), padding=(0,1,1)) # out:[1,4,4,4] #[1,1,5,4]
        self.conv7 = SNConv3d(channel, channel//4, kernel_size=(1,4,4), stride=1, padding=0) # out:[1,1,2,1]
        self.pool = nn.AdaptiveAvgPool3d((1,1,1))
    ...
    def forward(self, h, h_small, crop_idx):
        ...
        h_logit = self.fc2(h) # 64
        # print(h.shape)
        if self.num_class>0:
            h_class_logit = self.fc2_class(h)
            # print(h_class_logit.shape)  # bz, 7
            # print(h_class_logit.min())
...
class Sub_Generator(nn.Module): #(36, 44, 36)
...
class Generator(nn.Module):
    ...
    def forward(self, h, crop_idx=None, class_label=None): # output: 18, 
        ...
        if crop_idx != None or self.mode=='eval':
            ...
            h = F.interpolate(h,scale_factor = (2.25, 2.75, 2.25)) #36,44,36
            h = self.tp_conv4(h)
            h = self.relu(self.bn4(h))
            ...
            h = self.tp_conv5(h)
            h_latent = self.relu(self.bn5(h)) # (32, 32, 32), channel:128 #(36,44,36)

            #print(h_latent.shape)

            if self.mode == "train":
                h_small = self.sub_G(h_latent)
                h = h_latent[:,:,crop_idx//4:crop_idx//4+6,:,:] # Crop sub-volume, out: (4, 32, 32) # TODO: ours: (6,44,36)
            else:
                h = h_latent

        # Generate from latent feature
        h = F.interpolate(h,scale_factor = 2) # (6, 44, 36)

        h = self.tp_conv6(h)
        h = self.relu(self.bn6(h)) # (64, 64, 64) #(12,88,72)

        h = F.interpolate(h,scale_factor = 2) #  (2,1,24,176,144)
        h = self.tp_conv7(h)
