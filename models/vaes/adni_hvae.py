"HVAE implementation adapted from https://github.com/gulnazaki/counterfactual-benchmark"
    ...
    def __init__(
        ...
        if version == "light":  # uses less VRAM
            ...
            self.conv = nn.Sequential(
                activation,
                nn.Conv3d(in_width, bottleneck, kernel_size, 1, padding),
                activation,
                nn.Conv3d(bottleneck, out_width, kernel_size, 1, padding),
            )
        else: 
            activation = nn.GELU()
            self.conv = nn.Sequential(
                activation,
                nn.Conv3d(in_width, bottleneck, 1, 1),
                activation,
                nn.Conv3d(bottleneck, bottleneck, kernel_size, 1, padding),
                activation,
                nn.Conv3d(bottleneck, bottleneck, kernel_size, 1, padding),
                activation,
                nn.Conv3d(bottleneck, out_width, 1, 1),
            )

        if self.residual and (self.d or in_width > out_width):
            self.width_proj = nn.Conv3d(in_width, out_width, 1, 1)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.d:
            if isinstance(self.d, float):
                out = F.adaptive_avg_pool3d(out, int(out.shape[-1] / self.d))
            else:
                if out.shape[-1] == 144 and out.shape[-2] == 176:
                    out = F.adaptive_avg_pool3d(out, 96)
                else:
                    out = F.avg_pool3d(out, kernel_size=self.d, stride=self.d)
        return out
...
class DecoderBlock(nn.Module):
    def __init__(self, params, in_width: int, out_width: int, resolution: int):
        ...
        self.z_proj = nn.Conv3d(self.z_dim + params["context_dim"], in_width, 1)
        self.z_feat_proj = nn.Conv3d(self.z_dim + in_width, out_width, 1)
        self.conv = Block(
    ...
    def forward_prior(
        ...
        z = self.prior(z)
        p_loc = z[:, : self.z_dim, ...]                         # first 16 dim goes to p_loc

        p_logscale = z[:, self.z_dim : 2 * self.z_dim, ...]     # second 16 dim goes to p_logscale
        p_features = z[:, 2 * self.z_dim :, ...]                   # Other widths dim go to features
        if t is not None:
...
class Decoder(nn.Module):
    def __init__(self, params):
        ...
        for i, res in enumerate(self.all_res):
            if res <= params["bias_max_res"]:
                bias.append(
                    nn.Parameter(torch.zeros(1, params["widths"][::-1][i], res, res, res))
                )
    ...
    def forward(
        ...
        h = z = bias[1].repeat(parents.shape[0], 1, 1, 1, 1)  # initial state

        stats = []
        for i, block in enumerate(self.blocks):
            res = block.res  # current block resolution, e.g. 64x64
            pa = parents[..., :res, :res, :res].clone()  # select parents @ res
            if res == 144:
                pa = parents[..., :144, :176, :144].clone()
            if h.size(-1) < res:  # upsample previous layer output
                if res == 144:
                    b = bias[res] if res in bias.keys() else 0  # broadcasting
                    h = b + F.interpolate(h, size=(144,176,144))
                else:
                    b = bias[res] if res in bias.keys() else 0  # broadcasting
                    h = b + F.interpolate(h, scale_factor=res / h.shape[-1])

            # current prior depends on previous prior only, upsample previous prior latent z
            if z.size(-1) < res:
                if res == 144:
                    p_input = b + F.interpolate(z, size=(144,176,144))
                else:
                    p_input = b + F.interpolate(z, scale_factor=res / z.shape[-1])
            else:
                p_input = z
            p_loc, p_logscale, p_feat = block.forward_prior(p_input, pa, t=t) #compute prior p(zi|z<i)
            ...
            if (i + 1) < len(self.blocks):     # last block
...
class DGaussNet(nn.Module):
    def __init__(self, params):
        ...
        if params["input_channels"] == 3:
            self.channel_coeffs = nn.Conv3d(params["widths"][0], 3, kernel_size=1, stride=1)
