...
class Encoder(nn.Module):
    def __init__(self, cond_dim, latent_dim, hidden_dim, n_chan=[1, 16, 32, 64, 128, 256], stride=[1, 1, 1, 1, 1],
                 kernel_size=[3, 3, 3, 3, 3], padding=[1, 1, 1, 1, 1], input_size=(1, 144, 176, 144), num_convolutions=3, version='heavy'):
        super().__init__()
        ...
        for i in range(len(n_chan) - 1):
            for j in range(0, num_convolutions):
                ...
                layers[f'enc_{i+1}_conv_{j}'] = nn.Conv3d(in_channels=in_ch, out_channels=n_chan[i+1],
                                                          kernel_size=k, stride=s, padding=padding[i])
                layers[f'enc_{i+1}_batchnorm_{j}'] = nn.BatchNorm3d(n_chan[i+1])
...
class Decoder(nn.Module):
    def __init__(self, cond_dim, latent_dim, hidden_dim, n_chan=[256, 128, 64, 32, 16], stride=[1, 1, 1, 1, 1],
                 kernel_size=[4, 3, 3, 3, 3], padding=[1, 1, 1, 1, 1], output_size=(1, 144, 176, 144), num_convolutions=3, version='heavy'):
        super().__init__()
        self.latent_dim = latent_dim
        self.version = version
        if self.version == 'light':
            n_chan = [256, 128, 64, 32]
            kernel_size = [3,3,3,3]
        n_chan.append(self.latent_dim)
        self.n_chan = n_chan
        self.cond_dim = cond_dim
        self.hidden_dim = hidden_dim
        ...
        for i in range(len(n_chan) - 1):
            for j in range(0, num_convolutions):
                ...
                if j < num_convolutions - 1:
                    layers[f'dec_{i+1}_conv_{j}'] = nn.Conv3d(in_channels=in_ch, out_channels=n_chan[i+1],
                                                              kernel_size=3, stride=1, padding=1)
                else:
                    layers[f'dec_{i+1}_conv_{j}'] = nn.ConvTranspose3d(in_channels=in_ch, out_channels=n_chan[i+1],
                                                                       kernel_size=kernel_size[i] + 1, stride=stride[i] + 1, padding=padding[i])
                layers[f'dec_{i+1}_batchnorm_{j}'] = nn.BatchNorm3d(n_chan[i+1])
...
class ADNI3dCondVAE(CondVAE):
    def __init__(self, params, attr_size, name="image_vae", unconditional=False, beta_overwrite=None):
        # dimensionality of the conditional data
        cond_dim = params["context_dim"]
        latent_dim = params["latent_dim"]
        hidden_dim = params["hidden_dim"]
        version = params["version"]
        beta = beta_overwrite if beta_overwrite else params["beta"]
        lr = params["lr"]
        weight_decay = params["weight_decay"]
        fixed_logvar = params["fixed_logvar"]

        encoder = Encoder(cond_dim, latent_dim, hidden_dim, version=version)
        decoder = Decoder(cond_dim, latent_dim, hidden_dim, version=version)
