"""GAN architecture, adapted from https://github.com/gulnazaki/counterfactual-benchmark"""
import sys
sys.path.append("../../")
...
from datasets.adni_3d.dataset import ordinal_array
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, args):
        ...
        # Input size
        self.H, self.W, self.L = self.args['input_size']
        # Number of filters
        ...
        if self.normalization == 'batchnorm':
            self.normlayer = lambda x1,x2,x3,x4: nn.BatchNorm3d(x1)
            # self.normlayer = lambda x: nn.BatchNorm3d(x)
        elif self.normalization == 'layernorm':
            self.normlayer = lambda x1,x2,x3,x4: nn.LayerNorm([x1,x2,x3,x4])
            # self.normlayer = lambda x: nn.LayerNorm([x, self.H, self.W, self.L])
        else:
            self.normlayer = lambda x1,x2,x3,x4: nn.Identity()
...
class ConvBlock(BasicBlock):
    ...
    def __init__(self, args) -> None:
        ...
        self.block = nn.Sequential(
            ...
            self.normlayer(self.OF, self.H, self.W, self.L),
            self.nonlinearity,
        )
...
class UCB(BasicBlock):
    ...
    def __init__(self, args):
        ...
        self.step1 = nn.ConvTranspose3d(
            self.IF, self.OF, kernel_size=3, stride=2, padding=1, output_padding=1, bias=False)
...
class RandomFourierFeatures:
    ...
    def __call__(self, x):
        ...
        return x_fourier          # (bz, embedding_size * 2)
...
class TransformerXia(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.attr_size_dict = {var: size for var, size in self.attr_size_dict.items() if var in ["fro","par","tem","occ","cin","ins","ven"]}
        ...
        self.dec = nn.Sequential(
            nn.Linear(self.latent_dim, self.image_size[0]*self.image_size[1]*self.image_size[2]*self.OF),
            nn.BatchNorm1d(self.image_size[0]*self.image_size[1]*self.image_size[2]*self.OF)
        )
    ...
    def forward(self, x, cond, do_enc=False, enc_back=False, give_z=None):
        ...
        cond = self.encoding_operation(cond)      # (bz, 4)
        ...
        if give_z is not None:
            z = self.trans2(torch.cat((z, give_z), dim=1))
        out = self.dec(z)

        if enc_back:
            return self.enc_back(out)

        out = torch.reshape(out, (x.size(0), self.OF, self.image_size[0], self.image_size[1], self.image_size[2]))
...
class Encoder(nn.Module):
    def __init__(self, args):
        ...
        self.kernel_size = self.args['kernel_size']
        # Input shape
        self.input_shape = np.asarray(self.args['input_shape'])   # 144,176,144
        # number of features to start with
        self.num_feat = args['initial_filters']    # 32
        # Latent space and disease vectors dimension
        self.latent_dim = self.args['latent_space_dim']   # 512
        self.n_channels = self.args['n_channels']    # 1
        ...
        self.enc1 = nn.Sequential(
            nn.Conv3d(1, self.num_feat, self.kernel_size, stride=1, padding=1, bias=False),
            self.nonlinearity,
        ...
        self.trans = TransformerXia(
            ...
            'image_size': self.input_shape // 16,   # (9,11,9)
    ...
    def forward(self, x, cond):
        ...
        u = self.trans(enc5, cond, do_enc=True, enc_back=True)
        return u, enc1, enc2, enc3, enc4, enc5
...
class Decoder(nn.Module):
    def __init__(self, args):
        ...
        self.trans = TransformerXia(
            ...
            'IF': self.num_feat*8, 'OF': self.num_feat,
            'latent_dim': self.latent_dim,
            'image_size': self.input_shape // 16,
            'activation' : self.nonlinearity,
            'extra_latent': False,
            'attr_size_dict': self.args['attr_size_dict']}
        )
        ...
        self.dec4 = UCB({
            'input_size': self.input_shape//8, 'nonlinearity': self.nonlinearity,
            'normalization': self.normalization, 'kernel_size': self.kernel_size,
            'IF': self.num_feat * 9, 'OF': self.num_feat*4})
        self.dec3 = UCB({
            'input_size': self.input_shape//4, 'nonlinearity': self.nonlinearity,
            'normalization': self.normalization, 'kernel_size': self.kernel_size,
        ...
        self.dec0 = nn.Sequential(
            nn.Conv3d(self.num_feat, self.num_feat, self.kernel_size, stride=1, padding=1, bias=False),
            nn.BatchNorm3d(self.num_feat),
            self.nonlinearity,
            nn.Conv3d(
    ...
    def forward(self, u, cond, enc1, enc2, enc3, enc4, enc5):
        '''Call function.'''
        batch_dim = u.shape[0]
        u = u.squeeze()
        if batch_dim == 1:
        ...
        # Decoding path
        dec5 = torch.cat((u_cond, enc5), dim=1)
        dec4 = (self.dec4(dec5) + enc4) / 2
        dec3 = (self.dec3(dec4) + enc3) / 2
        dec2 = (self.dec2(dec3) + enc2) / 2
        dec1 = (self.dec1(dec2) + enc1) / 2
...
class Discriminator(nn.Module):
    def __init__(self, args):
        ...
        self.judge = nn.Sequential(
            nn.Dropout3d(0.2),
            nn.Conv3d(self.num_feat*(8+1), self.num_feat*8,
                      self.kernel_size, stride=1, padding=1, bias=False),
            self.nonlinearity,
            nn.Dropout3d(0.2),
            nn.Conv3d(self.num_feat*8, self.num_feat*8,
                      self.kernel_size, stride=1, padding=1, bias=False),
            self.nonlinearity,
            nn.Dropout3d(0.2),
            nn.Conv3d(self.num_feat*8, self.num_feat*8,
            ...
            self.nonlinearity,
            nn.Dropout3d(0.2),
            nn.Conv3d(self.num_feat*8, 1,
                      self.kernel_size, stride=1, padding=1, bias=False),
            nn.AvgPool3d(self.input_shape[0] // 16) # Average pooling per feature to do global average pooling
            # (BS, 1, 1, 1)
        )
